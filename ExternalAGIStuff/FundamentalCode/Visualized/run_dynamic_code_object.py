...
for j in range(input0.size):
    ...
    assert reg5<j> != 'dc::signal_break'
    if reg5<j> == 'dc::signal_return':
...
if input0 == 'dcr::reg':
    ...
    assert input1.'dc::runtime_registers'.exist((target.'dc::index' === reg0 and 'func::list_equal'(target.'dc::child_indices', reg1)))
    return input1.'dc::runtime_registers'.find((target.'dc::index' === reg0 and 'func::list_equal'(target.'dc::child_indices', reg1))).'value'
...
if input0 == 'dcr::call':
    reg0 &= input0.'dc::function_params'
    reg1 &= 'list'
    for i in range(reg0.size):
        reg1.append('func::solve_expression'(reg0[i], input1, None))
    return 'func::call_func'(input0.'dc::function_name', reg1)
...
if (input0 == 'dcr::assign' or input0 == 'dcr::assign_as_reference'):
    ...
    if input0.'dc::left_value' == 'dcr::reg':
        ...
        for i in range(input0.'dc::left_value'.'dc::child_indices'.size):
            reg1[i] &= 'func::solve_expression'(input0.'dc::left_value'.'dc::child_indices'[i], input1, None)
        assert not input1.'dc::runtime_registers'.exist((target.'dc::index' === input0.'dc::left_value'.'dc::index' and 'func::list_equal'(target.'dc::child_indices', reg2)))
...
if input0 == 'dcr::for':
    ...
    reg1.'value' &= 0
    input1.'runtime_iterators'.append(reg1)
    for i in range(reg0):
...
if input0 == 'dcr::if':
    if 'func::solve_expression'(input0.'dc::expression_for_judging', input1, None):
        ...
        for i in range(input0.'dc::elif_modules'.size):
            if 'func::solve_expression'(input0.'dc::elif_modules'[i].'expression_for_judging', input1, None):
                reg0<i> &= input0.'dc::elif_modules'[i].'elif_block'
...
if input0 == 'dcr::remove':
    reg0 &= 'func::solve_expression'(input0.'dc::target_list', input1, None)
    while True:
        reg1 &= 'func::remove_one_element'(reg0, input0.'dc::expression_for_constraint', input1)
        if not reg1:
            break
    return 'dc::line_signal_normal'
...
if input0 == 'dcr::call_none_return_func':
    ...
    reg1 &= 'list'
    for i in range(reg0.size):
        reg1.append('func::solve_expression'(reg0[i]))
    reg2 &= 'func::call_func'(input0.'dc::function_name', reg1)
    assert reg2 == None
    return 'dc::line_signal_normal'
assert False
'''